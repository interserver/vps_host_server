...
while [ ${spent} -lt ${maxtime} ]; do
	...
	#curl --connect-timeout 60 --max-time 600 -k -F action=cpu_usage -F "cpu_usage=${cpu_usage}" "https://myvps.interserver.net/vps_queue.php" 2>/dev/null;
	curl --connect-timeout 60 --max-time 600 -k -F action=cpu_usage -F "cpu_usage=${cpu_usage}" "http://myvps.interserver.net:55151/queue.php" 2>/dev/null;
	new=$(date +%s);
	lastspent=$((${new} - ${prev}));
	prev=$new;
