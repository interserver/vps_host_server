...
function get_vps_iptables_traffic($ips)
{
	...
	foreach ($ips as $ip => $id) {
		...
		if (sizeof($lines) == 2) {
			...
			//echo "$ip $in $out\n";
			$total = $in + $out;
//			$total = intval(trim(`export PATH="/usr/local/bin:/usr/local/sbin:\$PATH:/bin:/usr/bin:/sbin:/usr/sbin"; iptables -nvx -L FORWARD 2>/dev/null | grep -v DROP | grep " $ip " | tr -s [:blank:] |cut -d' ' -f3| awk '{ sum += $1 } END { print sum; }'`));
			if ($total > 0) {
