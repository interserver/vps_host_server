...
function get_qs_iptables_traffic($ips)
{
	...
	foreach ($ips as $ip => $id) {
		if ($vzctl == '') {
			$lines = explode("\n", trim(`export PATH="/usr/local/bin:/usr/local/sbin:\$PATH:/bin:/usr/bin:/sbin:/usr/sbin"; ebtables -L --Lc --Lx | grep " $ip -j CONTINUE -c " |  sed s#"ebtables -t filter -A FORWARD -p IPv4 --ip-... $ip -j CONTINUE -c "#""#g | cut -d" " -f2`));
		} else {
			$lines = explode("\n", trim(`export PATH="/usr/local/bin:/usr/local/sbin:\$PATH:/bin:/usr/bin:/sbin:/usr/sbin"; iptables -nvx -L FORWARD 2>/dev/null | awk '{ print " " $7 " " $8 " " $2 }' | grep -vi "[a-z]" | sort -n | grep " $ip " | awk '{ print $3 }'`));
		}
		if (sizeof($lines) == 2) {
			list($in, $out) = $lines;
			//echo "$ip $in $out\n";
			$total = $in + $out;
//			$total = intval(trim(`export PATH="/usr/local/bin:/usr/local/sbin:\$PATH:/bin:/usr/bin:/sbin:/usr/sbin"; iptables -nvx -L FORWARD 2>/dev/null | grep " $ip " | tr -s [:blank:] |cut -d' ' -f3| awk '{ sum += $1 } END { print sum; }'`));
			if ($total > 0) {
	...
	if ($vzctl == '') {
		`PATH="/usr/local/bin:/usr/local/sbin:\$PATH:/sbin:/usr/sbin"  ebtables -Z`;
	} else {
