...
$cmd .= "if [ -e /usr/bin/timeout ]; then
	...
	./vncsnapshot -dieblank -compresslevel 0 -quality 70 -vncQuality 7 -jpeg -fps 5 -count 1 -quiet -encodings raw :\$(($port - 5900)) shot_{$port}.jpg >/dev/null 2>&1;
		...
		if (file_exists('/usr/bin/prlctl')) {
			$out = trim(`export PATH="\$PATH:/bin:/usr/bin:/sbin:/usr/sbin";prlctl list -a -o uuid,name,numproc,status,ip,hostname,type -H;`);
			preg_match_all('/^\s*(?P<uuid>[^\s]+)\s+(?P<ctid>[^\s]+)\s+(?P<numproc>[^\s]+)\s+(?P<status>[^\s]+)\s+(?P<ip>[^\s]+)\s+(?P<hostname>[^\s]+)\s+(?P<type>[^\s]+)/m', $out, $matches);
			$valid_ctids = array_values($matches['ctid']);
		...
		foreach ($matches['ctid'] as $key => $id) {
			if (isset($valid_ctids) && !in_array($id, $valid_ctids))
				continue;
			$server = array(
				'veid' => $id,
