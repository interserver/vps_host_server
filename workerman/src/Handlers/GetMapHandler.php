...
/**
* 'get_map' - hub pushed updated vps map data; write the vps.* map files (only on
* change), rebuild ebtables / xinetd vnc entries when needed, then refresh the vps list.
*/
class GetMapHandler implements MessageHandlerInterface
{
				...
				foreach ($lines as $vps) {
					if (preg_match("/^(.*{$vps}):(.*)$/m", $maps['vnc'], $matches)) {
						if (!file_exists('/etc/xinetd.d/'.$matches[0])) {
							Worker::safeEcho(exec("{$dir}/provirted.phar vnc setup {$matches[0]} {$matches[1]}"));
						}
					}
				}
			}
