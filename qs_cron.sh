...
if [ $old_cron -eq 1 ]; then
	...
	if [ $count -ge 2 ]; then
        ...
        curl -s --connect-timeout 60 --max-time 600 -k -d action=get_new_qs http://myvps.interserver.net:55151/queue.php 2>/dev/null > $dir/cron.cmd;
        ...
        curl -s --connect-timeout 60 --max-time 600 -k -d action=get_qs_queue http://myvps.interserver.net:55151/queue.php 2>/dev/null > $dir/cron.cmd;
		if [ "$(cat $dir/cron.cmd)" != "" ] && [ "$(grep "Session halted." $dir/cron.cmd)" = "" ]; then
