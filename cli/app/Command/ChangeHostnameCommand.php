...
class ChangeHostnameCommand extends Command {
	...
	public function execute($hostname, $newname) {
		...
		if ($pool == 'zfs') {
			echo `zfs rename vz/{$hostname} vz/{$newname}`;
		} else {
			echo `lvrename /dev/vz/{$hostname} vz/{$newname}`;
		}
		echo `virsh domrename {$hostname} {$newname}`;
		echo `virsh dumpxml {$newname} > /root/cpaneldirect/vps.xml`;
		echo `sed s#"{$hostname}"#{$newname}#g -i /root/cpaneldirect/vps.xml`;
		echo `virsh define /root/cpaneldirect/vps.xml`;
		echo `rm -fv /root/cpaneldirect/vps.xml`;
		...
		if (file_exists('/etc/apt')) {
			echo `systemctl restart isc-dhcp-server 2>/dev/null || service isc-dhcp-server restart 2>/dev/null || /etc/init.d/isc-dhcp-server restart 2>/dev/null`;
		} else {
			echo `systemctl restart dhcpd 2>/dev/null || service dhcpd restart 2>/dev/null || /etc/init.d/dhcpd restart 2>/dev/null`;
		}
		echo `rm -vf /etc/xinetd.d/{$hostname} /etc/xinetd.d/{$hostname}-spice`;
		echo `virsh start {$newname}`;
		echo `/root/cpaneldirect/vps_refresh_vnc.sh {$newname}`;
	}
