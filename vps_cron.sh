...
if [ $old_cron -eq 1 ]; then
	...
	if [ $count -ge 2 ]; then
		...
		queue_request get_new_vps http://myvps.interserver.net:55151/queue.php --connect-timeout 60 --max-time 600 -k >$dir/cron.cmd
		if [ "$(cat $dir/cron.cmd)" != "" ] && [ "$(grep "Session halted." $dir/cron.cmd)" = "" ]; then
			...
			cron_warn "Session halted. in get_new_vps response (post-decrypt) — not executing"
		fi
		...
		queue_request get_queue http://myvps.interserver.net:55151/queue.php --connect-timeout 60 --max-time 600 -k >$dir/cron.cmd
		if [ "$(cat $dir/cron.cmd)" != "" ] && [ "$(grep "Session halted." $dir/cron.cmd)" = "" ]; then
			echo "Get Queue Running:	$(cat $dir/cron.cmd)" >>$log
			. $dir/cron.cmd >>$log 2>&1
		elif [ "$(grep "Session halted." $dir/cron.cmd)" != "" ]; then
			cron_warn "Session halted. in get_queue response (post-decrypt) — not executing"
		fi
